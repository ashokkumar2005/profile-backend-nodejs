...
router.get('/download-resume', async (req, res) => {
  try {
    const profile = await getOrCreate();
    if (!profile.resume) return res.status(404).json({ error: 'No resume uploaded' });

    // Strip leading slash before joining — critical on Windows where path.join
    // treats a leading '/' as an absolute path root (replaces __dirname entirely)
    const relativePath = profile.resume.replace(/^\//, '');
    const resumePath = path.join(__dirname, '..', relativePath);
